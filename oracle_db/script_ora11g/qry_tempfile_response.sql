spool tempfile_response.log

select 'TEMPFILE_DATE;INSTANCE;TYPE;FILENAME;MIN_MS;AVG_MS;P90_MS;P95_MS;P99_MS;MAX_MS;THRESHOLD' extraction from dual;
...
with
  t0 as (
    select s.instance_number, s.snap_id, s.end_interval_time snap_time, t.contents, f.filename, startup_time,
      ...
      from dba_hist_tempstatxs f, dba_hist_snapshot s, dba_hist_tablespace t

...
WITH t0 AS
  (SELECT s.instance_number,s.snap_id,s.end_interval_time snap_time,f.filename,startup_time,
  ...
  (SELECT instance_number,
    ...
    avg_write_time_ms
  FROM t2
  WHERE (rank_read <= &top_items
  OR rank_write    <= &top_items)
  )
SELECT 'DATAFILE_DATE;INSTANCE;FILENAME;MINREAD_MS;AVGREAD_MS;P90READ_MS;P95READ_MS;P99READ_MS;MAXREAD_MS;MINWRITE_MS;AVGWRITE_MS;P90WRITE_MS;P95WRITE_MS;P99WRITE_MS;MAXWRITE_MS;THRESHOLD' extraction
FROM dual
UNION ALL
SELECT *
FROM
  (SELECT TO_CHAR(TRUNC(snap_time,&date_trunc),&extract_mask)
    ||';'||instance_number
    ||';'||REPLACE(filename,'+','')
    ...
    ||';'|| REPLACE(trim(TO_CHAR(percentile_disc(0.90) within GROUP (
  ORDER BY avg_read_time_ms),'999999999990.00')),'.',&decimal_caract)
    ||';'|| REPLACE(trim(TO_CHAR(percentile_disc(0.95) within GROUP (
  ORDER BY avg_read_time_ms),'999999999990.00')),'.',&decimal_caract)
    ||';'|| REPLACE(trim(TO_CHAR(percentile_disc(0.99) within GROUP (
  ORDER BY avg_read_time_ms),'999999999990.00')),'.',&decimal_caract)
    ...
    ||';'|| REPLACE(trim(TO_CHAR(percentile_disc(0.90) within GROUP (
  ORDER BY avg_write_time_ms),'999999999990.00')),'.',&decimal_caract)
    ||';'|| REPLACE(trim(TO_CHAR(percentile_disc(0.95) within GROUP (
  ORDER BY avg_write_time_ms),'999999999990.00')),'.',&decimal_caract)
    ||';'|| REPLACE(trim(TO_CHAR(percentile_disc(0.99) within GROUP (
  ORDER BY avg_write_time_ms),'999999999990.00')),'.',&decimal_caract)
    ...
    ||';'|| REPLACE(trim(TO_CHAR(&datafile_th,'990.00')),'.',&decimal_caract)
  FROM t3
  GROUP BY TRUNC(snap_time,&date_trunc),
    instance_number,
    filename
  ORDER BY TRUNC(snap_time,&date_trunc),
    instance_number,
    filename
  );
...
with
  ...
  t1 as (
    select instance_number, snap_id, snap_time, filename,
     ...
     or delta_phywrts > 0),
  t2 as (
  select instance_number, snap_id, snap_time, filename, avg_read_time_ms,avg_write_time_ms,
         rank() over (partition by instance_number, snap_id order by avg_read_time_ms desc) rank_read,
         rank() over (partition by instance_number, snap_id order by avg_write_time_ms desc) rank_write
    from t1),
  t3 as (
    select instance_number, snap_time, filename, avg_read_time_ms,avg_write_time_ms from t2 where (rank_read <= &top_items or rank_write <= &top_items))
    select 'TEMPFILE_DATE;INSTANCE;FILENAME;MINREAD_MS;AVGREAD_MS;P90READ_MS;P95READ_MS;P99READ_MS;MAXREAD_MS;MINWRITE_MS;AVGWRITE_MS;P90WRITE_MS;P95WRITE_MS;P99WRITE_MS;MAXWRITE_MS;THRESHOLD' extraction from dual
union all
select * from (
select to_char(trunc(snap_time,&date_trunc),&extract_mask)||';'||instance_number||';'||replace(filename,'+','')||';'||
       replace(trim(to_char(min(avg_read_time_ms),'999999999990.00')),'.',&decimal_caract)||';'||
       replace(trim(to_char(avg(avg_read_time_ms),'999999999990.00')),'.',&decimal_caract)||';'||
       ...
       replace(trim(to_char(&datafile_th,'990.00')),'.',&decimal_caract)
     from t3
     group by trunc(snap_time,&date_trunc), instance_number, filename
     order by trunc(snap_time,&date_trunc), instance_number, filename);
...
from (select snap.end_interval_time snap_time, metric.instance_number, metric.metric_id, metric.metric_name,
       ...
       where metric.dbid = snap.dbid and metric.snap_id = snap.snap_id
         ...
         and snap.end_interval_time >= to_date(&begin_date, &date_mask)
         and snap.end_interval_time <= to_date(&end_date, &date_mask)
         and metric_name in ('Database CPU Time Ratio','CPU Usage Per Sec','Background CPU Usage Per Sec','Active Parallel Sessions','Active Serial Sessions','Average Active Sessions',
...
-- COMANDO ALTERADO... RETIREI A TABELA dba_hist_tablespace

with
  t0 as (
    select snap.snap_id, snap.end_interval_time snap_time,
           dba_tab.tablespace_name, trunc(((tsu.tablespace_usedsize * dba_tab.block_size) / (tsu.tablespace_size * dba_tab.block_size))*100, 2) perc,
           trunc((tsu.tablespace_size * dba_tab.block_size)/1024/1024/1024) total_gb
      from dba_hist_tbspc_space_usage tsu, dba_hist_snapshot snap, dba_tablespaces dba_tab
...
union all
select * from (
select to_char(trunc(snap_time,&date_trunc),&extract_mask)||';'||tablespace_name||';'||
  ...
  from t0
group by trunc(snap_time,&date_trunc), tablespace_name
order by trunc(snap_time,&date_trunc), tablespace_name);
